...
def plot_polar_graph(df: pd.DataFrame, prof_list: List[str], top_n: int = 30) -> go.Figure:
    skill_set = []
    ...
    for _, prof_name in enumerate(prof_list):
        prof_sum = df_copy[prof_name].sum()

        percentage_values = round((df_copy[prof_name] / prof_sum) * 100)
...
polar_fig = plot_polar_graph(
    df_plot_polar, prof_list=profession_list, top_n=25)
...
app.layout = html.Div(
    children=[
        html.H1(
            'Добро пожаловать в SkillMap: Ваш путеводитель в мир навыков и профессий'),
...
def update_chart(selected_preset,
    ...
    polar_fig = plot_polar_graph(
        df_plot_polar, prof_list=prof_list, top_n=num_skills)
    charts.append(dcc.Graph(id='polar-plot',
                  figure=polar_fig, style={'width': '90vh'}))
